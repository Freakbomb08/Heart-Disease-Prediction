import { useState } from "react";
import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Heart, Mail, Lock, User, ArrowLeft, Github } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

interface AuthPageProps {
  mode: "signin" | "signup";
}

const AuthPage = ({ mode: initialMode }: AuthPageProps) => {
  const [mode, setMode] = useState(initialMode);
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [name, setName] = useState("");
  const { toast } = useToast();

  const isSignIn = mode === "signin";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isSignIn) {
      toast({
        title: "Sign In Attempted",
        description: `Logging in user: ${email}`,
        duration: 2000,
      });
    } else {
      toast({
        title: "Sign Up Successful (Mock)",
        description: `Account created for ${name}. Please sign in.`,
        duration: 3000,
      });
      setMode("signin");
    }
  };

  const toggleMode = () => {
    setMode(isSignIn ? "signup" : "signin");
  };

  const formTitle = isSignIn ? "Welcome Back" : "Create Account";
  const formDescription = isSignIn ? "Enter your credentials to access the personalized dashboard." : "Join CardioPredict to save your risk reports and monitor progress.";
  const submitButtonText = isSignIn ? "Sign In" : "Sign Up";
  const toggleText = isSignIn ? "Don't have an account?" : "Already have an account?";
  const toggleLinkText = isSignIn ? "Sign up" : "Sign in";

  return (
    <div className="flex min-h-screen items-center justify-center bg-muted/50 p-4">
      <Link to="/" className="absolute top-4 left-4 text-sm text-muted-foreground hover:text-foreground flex items-center">
        <ArrowLeft className="w-4 h-4 mr-1" />
        Back to Home
      </Link>

      <Card className="w-full max-w-md shadow-2xl rounded-xl">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-2">
            <Heart className="w-8 h-8 text-red-600 fill-red-600" />
          </div>
          <CardTitle className="text-3xl font-bold">{formTitle}</CardTitle>
          <CardDescription>{formDescription}</CardDescription>
        </CardHeader>
        
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            
            {/* Name Input (Only for Sign Up) */}
            {!isSignIn && (
              <div className="space-y-2">
                <Label htmlFor="name">Name</Label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <Input 
                    id="name" 
                    type="text" 
                    placeholder="Full Name" 
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    required={!isSignIn}
                    className="pl-10"
                  />
                </div>
              </div>
            )}
            
            {/* Email Input */}
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input 
                  id="email" 
                  type="email" 
                  placeholder="name@example.com" 
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="pl-10"
                />
              </div>
            </div>

            {/* Password Input */}
            <div className="space-y-2">
              <Label htmlFor="password">Password</Label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input 
                  id="password" 
                  type="password" 
                  placeholder="••••••••" 
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  className="pl-10"
                />
              </div>
            </div>
            
            {/* Forgot Password Link (Only for Sign In) */}
            {isSignIn && (
              <div className="flex justify-end">
                <Link to="/forgot-password" className="text-sm font-medium text-teal-600 hover:text-teal-700">
                  Forgot password?
                </Link>
              </div>
            )}

            {/* Submit Button */}
            <Button type="submit" className="w-full bg-teal-500 hover:bg-teal-600">
              {submitButtonText}
            </Button>
            
            {/* Toggle Link */}
            <p className="text-center text-sm text-muted-foreground">
              {toggleText}
              <button 
                type="button" 
                onClick={toggleMode} 
                className="ml-1 font-medium text-teal-600 hover:text-teal-700 underline"
              >
                {toggleLinkText}
              </button>
            </p>
            {/* OAuth */}
            <div className="mt-4 flex flex-col gap-2">
                <a
                    href="http://localhost:8080/auth/google"
                    className="w-full bg-white text-black py-2 rounded-lg flex items-center justify-center"
                >
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                </svg>
                Continue with Google
            </a>
            <a
                href="http://localhost:8080/auth/github"
                className="w-full bg-gray-800 text-white py-2 rounded-lg flex items-center justify-center"
>
                <Github className="w-5 h-5 mr-2" />
                Continue with GitHub
            </a>
            </div>
          </form>
        </CardContent>
      </Card>
    </div>
  );
};

export default AuthPage;